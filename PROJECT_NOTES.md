...
- **Десктоп:** мышь — выбор (клик/рамка), ПКМ/клик по земле — приказ движения, по врагу — атака;
  ...
  - ⚠️ На мобилке ПК-управление **отключено**: `isMobileRef` (live-зеркало `isMobile`) гейтит
    `handleMouseDown/MoveSelection/Up` и edge-scroll. Иначе тач-браузер шлёт синтетические
    mouse-события после тача → двойная обработка (ПК + тач одновременно).
...
- **Нижняя командная панель**: миникарта (радар) + супероружие + контекстное производство
  (builder→здания, CC→строители, barracks/war_factory→юниты).
- **Десктоп vs мобилка** (`useIsMobile()` из `src/utils/useIsMobile.ts` = touch || ширина ≤1024):
  - Десктоп — прежняя нижняя панель (`{!isMobile && (...)}`).
  - Мобилка — отдельный **док-оверлей** (`{isMobile && (...)}`) поверх фулскрин-карты: свёрнут =
    только полупрозрачная стрелка снизу; развёрнут = компактная панель (радар 84px + производство
    + строка супероружия). Камера на мобилке ближе (`sim.camZoom = isMobile ? 15 : 22`).
  - Контент радара/производства вынесен в `renderMinimapBox(cls)` / `renderProductionContent(compact)`
    и переиспользуется обеими раскладками. При `compact` кнопки получают класс `.prod-btn-compact`
    (меньше высота/шрифт, см. index.css) и более плотные сетки в landscape (`grid-cols-4/6`),
    чтобы в горизонтали все кнопки заказа влезали в док без переполнения.
- **Тултип постройки** отвязан от меню: `bindTooltip(type,key)` навешивает ховер (ПК — карточка
  бежит за курсором) и long-press ~300мс (мобилка — карточка у пальца, короткий тап = постройка).
  Рендерится плавающим `fixed`-блоком в конце компонента (`hoveredItem && tooltipVisible`).
- Плавающая панель действий моб. (выбор армии/сброс/отмена) — сверху по центру (`top-[58px]`).
- Миникарта: враги/нейтралы рисуются только где есть видимость (`fogQueryRef`).
- **Главное меню (`LobbyPanel`)** имеет отдельную мобильную ветку (`if (isMobile) return ...`):
  **пошаговый мастер** (`mobileStep`: `mode → faction → config|network`) с индикатором-точками
  и футером Назад/Далее/«В бой». Шаги: 1) режим (одиночная/мультиплеер, тап по карте = выбор+переход),
  2) фракция (2×2 портрет / `landscape:grid-cols-4`), 3a) config — своя команда + слайдер врагов +
  команды врагов («В бой» в футере, всегда виден), 3b) network — подбор/комната.
  Хелперы `mobilePickMode/mobileGoNext/mobileGoBack`, `mobileSteps/mobileStepIndex`. Активное лобби —
  отдельная компактная раскладка. Всё в `100dvh overflow-hidden`, футер `shrink-0` → кнопки видны
  и в горизонтали. Десктопный JSX ниже без изменений.
